...
def ComputePoseArrayDistSqr(pose0, posearray, quatweight=1.0):
    ...
    """
    diff2 = numpy.abs(pose0 - posearray)**2
    qdists0 = numpy.sum(diff2[4:7])
    qdists1 = numpy.sum((pose0[0:4]-posearray[0:4])**2)
    return numpy.minimum(qdists0, qdists1)*quatweight + numpy.sum(diff2[4:7])
